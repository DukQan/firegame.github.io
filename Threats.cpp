...
void Threats::ResetBullet(Bullet* p_bullet)
{
...
void Threats::MakeBullet(SDL_Renderer* des, const int& x_limit, const int& y_limit)
{
    if (is_alive_)
    {
        for (int i = 0; i < bullet_list_.size(); i++)
...
void Threats::SetAnimationPos(const int& pos_x_a, const int& pos_x_b)
{
    ...
    else
    {
        if (on_ground_ == true)
        {
            if (x_pos_ > animation_b_)
            {
                ...
                LoadImg("img//threat_left2.png", screen);
            }
            else if (x_pos_ < animation_a_)
            {
                input_type_.right_ = 1;
                input_type_.left_ = 0;
                LoadImg("img//threat_right2.png", screen);
            }
        }
        else
        {
...
void Threats::DoPlayer(Map& g_map)
{
    if (think_time_ == 0)
    {
        x_val_ = 0;
...
void Threats::CenterEntityOnMap(Map& g_map)
{
...
void Threats::CheckToMap(Map& g_map)
{
    int x1 = 0;
    int x2 = 0;
    int y1 = 0;
